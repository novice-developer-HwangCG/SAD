...
class TestApp:
    def __init__(self, root):
        ...
        try:
            ...
            self.diar_sw_cover = tk.PhotoImage(file="C:/code_img_folder/diar_sw0_0.png")          # 새로 추가한 이미지 스위치 덮개 이미지
        except tk.TclError:
    ...
    def init_widgets(self):
        ...
        for i, var_name in enumerate(["ARM_1", "ARM_2", "ARM_3", "FIRE"]): 
            ...
            label = tk.Label(self.root, text=f"{var_name} = OFF", font=("Arial", 14))
        ...
        #DIAR 버튼 위치
        self.DIAR_label = tk.Label(self.root, text="DIAR = OFF", font=("Arial", 14))
    ...
    def draw_signal(self, signal_data, y_high, y_low):  # 디지털 신호 그리기
        ...
        step = 390 / len(signal_data)   # 각 데이터 포인트를 x축 상에 균등하게 분포시키기 위해 사용, 390 = x축의 총 길이 -> 각 데이터 포인트는 390/10 = 39 만큼의 x축 간격을 가짐
