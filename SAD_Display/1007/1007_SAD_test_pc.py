...
class ImageToggleButton(tk.Frame):
    ...
    def __init__(self, root, on_image_path, off_image_path, variable_name, label, *args, **kwargs):
        ...
        # 각 버튼마다 상태를 인스턴스 변수로 관리
        self.is_on = False  # False는 OFF 상태를 의미

    def toggle(self):
        """버튼 제어 로직 (ARM_1이 OFF일 때 ARM_2는 ON 불가능 등)"""
        ...
        if self.variable_name == "ARM_1":
            ImageToggleButton.ARM_1 = self.is_on
            self.arm_1_action()
        elif self.variable_name == "ARM_2":
            ImageToggleButton.ARM_2 = self.is_on
            self.arm_2_action()
        elif self.variable_name == "ARM_3":
            ImageToggleButton.ARM_3 = self.is_on
            self.arm_3_action()
        elif self.variable_name == "FIRE":
            ImageToggleButton.FIRE = self.is_on
            self.fire_action()
        elif self.variable_name == "DIAR":
            ImageToggleButton.ARM_DIAR = self.is_on
            self.diar_action()

        self.update_button_display()

    def arm_1_action(self):
        """ARM_1 버튼이 켜지면 수행할 기능"""
        if self.is_on:
            self.send_signal_to_pico("ARM_1", True)
        else:
            self.send_signal_to_pico("ARM_1", False)

    def arm_2_action(self):
        """ARM_2 버튼이 켜지면 수행할 기능"""
        if self.is_on:
            print("ARM_2 활성화")
        else:
            print("ARM_2 비활성화")

    def arm_3_action(self):
        """ARM_3 버튼이 켜지면 수행할 기능"""
        if self.is_on:
            print("ARM_3 활성화")
        else:
            print("ARM_3 비활성화")
    ...
    def update_button_display(self):
        """버튼 이미지, 신호 그래프, 라벨을 업데이트하는 함수."""
        
        # 상태에 따라 버튼 이미지 및 텍스트 변경
        if self.is_on:
...
class SignalData(tk.Frame):
    ...
    def update_loop(self):
        ...
        self.update_signal_history(self.signal_Data1, 1 if ImageToggleButton.ARM_1 else 0)
        self.update_signal_history(self.signal_Data2, 1 if ImageToggleButton.ARM_2 else 0)

        if ImageToggleButton.ARM_3:
    ...
    def update_signal_labels(self):
        # 상태에 따라 텍스트 라벨 업데이트
        self.signal_labels[0].config(text="HIGH" if toggle_buttons[0].is_on else "LOW")
        self.signal_labels[1].config(text="HIGH" if toggle_buttons[1].is_on else "LOW")
        self.signal_labels[2].config(text="HIGH" if toggle_buttons[2].is_on else "LOW")
...
class DiarToggleButton(tk.Frame):
    ...
    def reset_all(self):
        ...
        for button in toggle_buttons:
            ...
            button.is_on = False
            button.button.config(image=button.off_image)  # 버튼 이미지 OFF로 설정
            button.label.config(text=f"{button.variable_name} = OFF")  # 라벨 업데이트
        ...
        # 신호 데이터를 0으로 설정 (신호 리스트에 값을 직접 추가)
        signal_graph.update_signal_history(signal_graph.signal_Data1, 0)
        signal_graph.update_signal_history(signal_graph.signal_Data2, 0)
        signal_graph.update_signal_history(signal_graph.signal_Data3, 0)
...
# 신호 그래프 인스턴스 생성
signal_graph = SignalData(root, canvas_inside_medium, signal_labels, fuze_var)
